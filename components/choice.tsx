...
export default function Choice(props: { onClick: () => void, label: string, votes: number, percentage: number, showResults: boolean, index: 0 | 1, winner: boolean }) {
  ...
  return (
    <div ref={ref} className={`flex flex-col items-center justify-center gap-2 transition-all duration-500 ${!props.showResults ? "hover:bg-secondary cursor-pointer" : (props.winner ? "bg-green-900" : "bg-red-900")} absolute z-0 overflow-hidden w-full lg:h-full ${props.showResults ? `h-[var(--percentage)] lg:w-[var(--percentage)] ${props.index && `top-[calc(100%-var(--percentage))] lg:left-[calc(100%-var(--percentage))] lg:top-0`}` : `h-1/2 lg:w-1/2 ${props.index ? "top-1/2 left-0 lg:left-1/2 lg:top-0" : "top-0 left-0"}`}`} onClick={() => { props.onClick() }}>
